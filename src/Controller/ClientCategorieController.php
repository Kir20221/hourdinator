...
class ClientCategorieController extends AbstractController
{
    ...
    public function new(Request $request, ClientCategorieRepository $clientCategorieRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $clientCategorieRepository->add($clientCategorie);
    ...
    /**
     * @Route("/{id}", name="app_client_categorie_delete", methods={"POST"})
     */
    public function delete(Request $request, ClientCategorie $clientCategorie, ClientCategorieRepository $clientCategorieRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$clientCategorie->getId(), $request->request->get('_token'))) {
            $clientCategorieRepository->remove($clientCategorie);
        }
